...
@section('content')
    ...
    ?>
    <h1 class="ls-title-intro ls-ico-home">Olá {{ Auth::user()->name }}! Seja bem vindo ao WebCar</h1>

    <div class="ls-box ls-board-box">
        <header class="ls-info-header card-footer">
            <h2 class="ls-title-3">Dashboard Mês Atual: <a style="color:rgb(77, 178, 209);">{{ date('m / Y') }}</a>
            </h2>
                        ...
                        <div class="box-header">
                            <form method="post" action="{{ route('vehicle.list') }}" class="form form-inline">
                                ...
                                <button type="submit" class="btn btn-light"
                                    style="width: 150px; margin-left: auto; margin-right: auto; color: #000; font-weight: bold;">Exibir</button>
                            </form>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-sm-6 col-md-3 dash">
            <div class="ls-box" style="background-color: #C64B4D;">
                ...
                <div class="ls-box-body">
                    <strong>{{ App\Solicitacao::where('statussolicitacao', 'PENDENTE')->where('datasaida', '>', date('Y-m-d', mktime(0, 0, 0, date('m'), 1, date('Y'))))->where('solicitante', auth()->user()->id)->count() }}</strong>
                </div>
                <div class="ls-box-footer">
                    <div class="box-header">
                        <a href="{{ route('solicitacoes.pendentes') }}" class="btn btn-light"
                            style="width: 150px; margin-left: auto; margin-right: auto; color: #000; font-weight: bold;">Exibir</a>
                    </div>
                </div>
            </div>
        ...
        @can('View ADM dashboard')
            <div class="col-sm-6 col-md-3 dash">
                <div class="ls-box" style="background-color: #F7293D;">
                    ...
                    <div class="ls-box-body">
                        <strong>{{ App\Solicitacao::where('statussolicitacao', 'PENDENTE')->where('datasaida', '>', date('Y-m-d', mktime(0, 0, 0, date('m'), 1, date('Y'))))->count() }}</strong>
                    </div>
                    <div class="ls-box-footer">
                        <div class="box-header">
        ...
        <div class="col-sm-6 col-md-3 dash">
            <div class="ls-box" style="background-color: 	#00BFFF;">
                ...
                <div class="ls-box-body">
                    <strong>{{ App\Solicitacao::where('statussolicitacao', 'REALIZADO')->where('datasaida', '>', date('Y-m-d', mktime(0, 0, 0, date('m'), 1, date('Y'))))->where('solicitante', auth()->user()->id)->count() }}</strong>
                </div>
                <div class="ls-box-footer">
                    <div class="box-header">
    ...
    <div class="ls-box card-footer" style="margin-top: -21px;">
        <h5 style="margin-top: 8px; margin-bottom: 8px;">
            Não entende como prosseguir?&nbsp;&nbsp; Veja o &nbsp;
            <a href="/informacao/tutorial" style="font-size: 23px; text-decoration: none;">
                Tutorial do Sistema
            </a>
        </h5>
    </div>
